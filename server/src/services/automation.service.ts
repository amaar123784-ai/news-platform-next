...
export class AutomationService {
     ...
     * Step 1: AI Rewriting
     */
    private async processAIRewrite(queueId: string): Promise<void> {
     ...
     */
    public async markSocialPosted(queueId: string, postId: string): Promise<void> {
        await prisma.automationQueue.update({
            where: { id: queueId },
            data: {
     ...
     */
    public async markSocialFailed(queueId: string, errorMessage: string): Promise<void> {
        ...
        if (!shouldRetry) {
            await notificationService.createNotification(
                ...
                `فشل نشر المقال بعد 3 محاولات: ${errorMessage}`,
                { queueId }
            );
        }
    }
