 ...
 */
const SOURCE_TIERS: Record<string, { tier: 1 | 2 | 3; multiplier: number }> = {
...
const sourceBurstTracker: Map<string, BurstState> = new Map();
...
const fingerprintCache: FingerprintEntry[] = [];
 ...
 */
function extractSourceDomain(sourceName: string, sourceUrl?: string): string {
    if (sourceUrl) {
        try {
 ...
 */
function getSourceTier(sourceName: string, sourceUrl?: string): { tier: 1 | 2 | 3; multiplier: number } {
 ...
 * Check burst control
 */
function checkBurstControl(sourceId: string): { isBurst: boolean; count: number } {
    const now = new Date();
 ...
 */
function findSimilarArticle(description: string, sourceId: string): FingerprintEntry | null {
    const now = Date.now();
 ...
 */
function addToFingerprintCache(articleId: string, description: string, sourceId: string): void {
 ...
 */
export function processYemenFilter(item: RSSItemInput): FilterResult {
 ...
 */
export function clearBurstTracking(): void {
    const now = Date.now();
