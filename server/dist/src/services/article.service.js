 ...
 */
export async function getFeaturedArticles(limit) {
 ...
 */
export async function getRelatedArticles(idOrSlug, limit) {
...
export async function createArticle(data, userId) {
    ...
    if (!article)
        throw createError(500, 'فشل إنشاء المقال بعد عدة محاولات', 'SLUG_CONFLICT');
...
export async function updateArticle(id, data, user) {
    ...
    if (sanitized.seoTitle)
        sanitized.seoTitle = sanitizeText(sanitized.seoTitle);
    if (sanitized.seoDesc)
        sanitized.seoDesc = sanitizeText(sanitized.seoDesc);
 ...
 */
export async function restoreArticle(id, userId) {
 ...
 * Archive an article
 */
export async function archiveArticle(id, userId) {
    const article = await prisma.article.update({
        where: { id },
