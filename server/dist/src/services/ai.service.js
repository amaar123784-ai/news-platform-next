...
function cleanNonArabic(text) {
    ...
    cleaned = cleaned.replace(/(?<!["«])(\b[a-zA-Z]{3,}\b)(?!["»])/g, (match) => {
        ...
        if (allowed.some(w => w.toLowerCase() === match.toLowerCase()))
            return match;
        return '';
    });
 ...
 */
const TITLE_EXCERPT_PROMPT = (title, excerpt) => `
 ...
 */
const FULL_ARTICLE_PROMPT = (title, content, category) => `
