 ...
 */
export async function listPublicArticles(page, perPage, categoryId, categorySlug) {
...
export async function listSources() {
    ...
    return sources.map((source) => ({
        ...source,
        _count: {
            articles: source.feeds.reduce((sum, feed) => sum + feed._count.articles, 0),
...
export async function createSource(data, userId) {
    ...
    const existingFeeds = await prisma.rSSFeed.findMany({
        where: { feedUrl: { in: data.feeds.map((f) => f.feedUrl) } },
        select: { feedUrl: true }
    });
    if (existingFeeds.length > 0) {
        throw createError(400, `رابط RSS موجود مسبقاً: ${existingFeeds.map((f) => f.feedUrl).join(', ')}`, 'DUPLICATE_FEED_URL');
    }
...
export async function fetchSourceFeeds(sourceId) {
    ...
    const results = await Promise.all(feeds.map((f) => fetchRSSFeed(f.id)));
 ...
 */
export async function addFeed(sourceId, data) {
    // Check source exists
 ...
 */
export async function updateArticleStatus(id, status, userId) {
 ...
 */
export async function rewriteSingleArticle(id) {
    const article = await prisma.rSSArticle.findUnique({
        where: { id },
    });
    if (!article) {
 ...
 */
export async function bulkRewrite(ids) {
...
//# sourceMappingURL=rssAdmin.service.js.map