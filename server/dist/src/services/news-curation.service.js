...
function scoreArticle(article) {
    ...
    if (article.imageUrl)
        score += 1;
    if (article.category && LOCAL_SLUGS.includes(article.category.slug))
        score += 2;
    return score;
}
/**
 ...
 */
export async function refreshFeaturedArticles() {
    const now = new Date();
 ...
 */
export async function expireBreakingNews(maxAgeHours = 6) {
...
//# sourceMappingURL=news-curation.service.js.map