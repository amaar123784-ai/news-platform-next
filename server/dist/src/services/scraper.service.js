 ...
 */
async function fetchPageHTML(url, retries = 3) {
 ...
 */
function ensureAbsoluteUrl(imageUrl, baseUrl) {
    if (!imageUrl)
        return null;
    try {
 ...
 */
function extractWithSelectors(html, config) {
    try {
        const $ = cheerio.load(html);
 ...
 */
export async function scrapeArticle(articleId) {
    try {
 ...
 */
async function updateArticleScrapeStatus(articleId, content, error, newImageUrl = null) {
    const updateData = {
        fullContent: content,
 ...
 */
export async function processScrapeQueue(batchSize = 10) {
    ...
    const results = await Promise.all(articles.map(article => limit(async () => {
 ...
 */
export async function retryFailedScrapes(limit = 5) {
