...
describe('Article Publishing Integration Tests', () => {
    ...
    beforeAll(() => {
        ...
        adminToken = jwt.sign({ userId: mockAdmin.id, email: mockAdmin.email, role: mockAdmin.role }, secret, { expiresIn: '1h' });
        journalistToken = jwt.sign({ userId: mockJournalist.id, email: mockJournalist.email, role: mockJournalist.role }, secret, { expiresIn: '1h' });
        readerToken = jwt.sign({ userId: mockReader.id, email: mockReader.email, role: mockReader.role }, secret, { expiresIn: '1h' });
    });
    beforeEach(() => {
        app = createApp();
        vi.clearAllMocks();
        ...
        mockPrisma.user.findUnique.mockImplementation(({ where }) => {
            ...
            if (where.id === mockJournalist.id)
                return Promise.resolve(mockJournalist);
            if (where.id === mockReader.id)
                return Promise.resolve(mockReader);
    ...
    describe('POST /api/articles/:id/publish - Publish Article', () => {
        ...
        it('should allow admin to publish article', async () => {
            ...
            expect(mockPrisma.article.update).toHaveBeenCalledWith(expect.objectContaining({
...
//# sourceMappingURL=article.publish.integration.test.js.map