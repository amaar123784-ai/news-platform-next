...
import { ImageProcessor, imageFileFilter, ImageProcessingError } from '../services/imageProcessor.js';
const router = Router();
...
router.post('/upload', authenticate, requireRole('ADMIN', 'EDITOR', 'JOURNALIST'), upload.single('file'), async (req, res, next) => {
    try {
        ...
        if (!imageProcessor.validateMimeType(req.file.mimetype)) {
            throw createError(400, 'نوع الملف غير مدعوم. يُسمح فقط بالصور (JPEG, PNG, GIF, WEBP, AVIF)', 'INVALID_FILE_TYPE');
        }
        // Process and optimize image
        const processed = await imageProcessor.process(req.file.buffer, req.file.originalname);
        // Store in database
